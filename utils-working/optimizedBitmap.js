...
class OptimizedBitmapEditor {
    ...
    setPixel(x, y, value, pattern = null) {
        ...
        const operation = (pixels) => {
            ...
            if (pattern && typeof Patterns !== 'undefined') {
                pixels[index] = Patterns.applyPattern(pattern, x, y);
            } else {
                pixels[index] = value;
            }
        };
    ...
    floodFill(startX, startY, newValue, pattern = null) {
        ...
        // Early exit if trying to fill with the same value
        if (pattern && typeof Patterns !== 'undefined') {
            const patternValue = Patterns.applyPattern(pattern, startX, startY);
            if (originalValue === patternValue) return;
        ...
        while (stack.length > 0) {
            ...
            // Skip if already visited or not the original value
            if (visited[index] || activeLayer.pixels[index] !== originalValue) continue;
            ...
            // Apply pattern or solid color directly
            if (pattern && typeof Patterns !== 'undefined') {
                activeLayer.pixels[index] = Patterns.applyPattern(pattern, x, y);
            } else {
                activeLayer.pixels[index] = newValue;
            }
