class ImagePlacementDialog {
    constructor(bitsDraw) {
        ...
        this.ditherMethod = 'threshold';
        this.invert = false;
        this.alphaMode = 'ignore';
    ...
    setupEventListeners() {
        ...
        const ditherSelect = document.getElementById('dither-method-select');
        if (ditherSelect) {
            ditherSelect.addEventListener('change', (e) => {
                this.ditherMethod = e.target.value;
                this.updatePreview();
            });
        }

        // Invert colors checkbox
        const invertCheckbox = document.getElementById('invert-colors-checkbox');
        if (invertCheckbox) {
        ...
        // Alpha mode select
        const alphaModeSelect = document.getElementById('alpha-mode-select');
        if (alphaModeSelect) {
            alphaModeSelect.addEventListener('change', (e) => {
                this.alphaMode = e.target.value;
                this.updatePreview();
            });
        }
    ...
    async show(imageData) {
        ...
        if (ditherSelect) {
            ditherSelect.value = this.ditherMethod;
        }
        
        if (invertCheckbox) {
            invertCheckbox.checked = this.invert;
        }
        
        if (alphaModeSelect) {
            alphaModeSelect.value = this.alphaMode;
        }
    ...
    async updatePreview() {
        ...
        try {
            // Process image at current size with enhanced options
            const options = {
                threshold: this.threshold,
                alphaHandling: this.alphaHandling,
    ...
    async applyImage() {
        ...
        try {
            ...
            // Load the bitmap data into the new layer with alpha channel support
            for (let y = 0; y < Math.min(this.processedBitmap.height, this.app.editor.height); y++) {
                for (let x = 0; x < Math.min(this.processedBitmap.width, this.app.editor.width); x++) {
                    if (this.processedBitmap.pixels[y] && this.processedBitmap.pixels[y][x] !== undefined) {
                        ...
                        if (this.processedBitmap.alpha && this.processedBitmap.alpha[y] && this.processedBitmap.alpha[y][x] !== undefined) {
                            newLayer.alpha[index] = this.processedBitmap.alpha[y][x];
                        } else {
                            // Default to opaque if no alpha data
                            newLayer.alpha[index] = 1;
                        }
                    }
                }
            }
